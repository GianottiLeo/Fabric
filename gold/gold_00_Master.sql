...
BEGIN
    ...
    PRINT 'Step 1/8: Refreshing Gold Portfolio...';
    EXEC [_Gold].[usp_Refresh_Portfolio];
    SET @msg = CONCAT('  Completed in ', DATEDIFF(SECOND, @step_start, SYSUTCDATETIME()), 's');
    PRINT @msg;

    SET @step_start = SYSUTCDATETIME();
    PRINT 'Step 2/8: Refreshing Gold Business...';
    EXEC [_Gold].[usp_Refresh_Business];
    SET @msg = CONCAT('  Completed in ', DATEDIFF(SECOND, @step_start, SYSUTCDATETIME()), 's');
    PRINT @msg;

    SET @step_start = SYSUTCDATETIME();
    PRINT 'Step 3/8: Refreshing Gold Portfolio Row...';
    EXEC [_Gold].[usp_Refresh_Portfolio_Row];
    SET @msg = CONCAT('  Completed in ', DATEDIFF(SECOND, @step_start, SYSUTCDATETIME()), 's');
    PRINT @msg;

    SET @step_start = SYSUTCDATETIME();
    PRINT 'Step 4/8: Refreshing Gold Portfolio Aggregate...';
    EXEC [_Gold].[usp_Refresh_Portfolio_Aggregate];
    SET @msg = CONCAT('  Completed in ', DATEDIFF(SECOND, @step_start, SYSUTCDATETIME()), 's');
    PRINT @msg;

    SET @step_start = SYSUTCDATETIME();
    PRINT 'Step 5/8: Refreshing Gold Portfolio Bid...';
    EXEC [_Gold].[usp_Refresh_Portfolio_Bid];
    SET @msg = CONCAT('  Completed in ', DATEDIFF(SECOND, @step_start, SYSUTCDATETIME()), 's');
    PRINT @msg;

    SET @step_start = SYSUTCDATETIME();
    PRINT 'Step 6/8: Refreshing Gold Complaints...';
    EXEC [_Gold].[usp_Refresh_Complaints];
    SET @msg = CONCAT('  Completed in ', DATEDIFF(SECOND, @step_start, SYSUTCDATETIME()), 's');
    PRINT @msg;

    SET @step_start = SYSUTCDATETIME();
    PRINT 'Step 7/8: Refreshing Gold Complaint Aggregate...';
    EXEC [_Gold].[usp_Refresh_Complaint_Aggregate];
    SET @msg = CONCAT('  Completed in ', DATEDIFF(SECOND, @step_start, SYSUTCDATETIME()), 's');
    PRINT @msg;

    SET @step_start = SYSUTCDATETIME();
    PRINT 'Step 8/8: Refreshing Gold PostSale...';
    EXEC [_Gold].[usp_Refresh_PostSale];
    SET @msg = CONCAT('  Completed in ', DATEDIFF(SECOND, @step_start, SYSUTCDATETIME()), 's');
    PRINT @msg;
